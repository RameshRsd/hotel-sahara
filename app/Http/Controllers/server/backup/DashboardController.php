...
//        $roomCheck = RoomCheck::where('room_status','CheckedIn')->orderBy('id','DESC');
        $roomCheckStatusCheck = RoomCheck::where('room_status','CheckedIn')->orderBy('id','DESC');
        $this->data('title',$this->title('Dashboard'));

        return view($this->pagePath.'Admin.index',$this->data,compact('roomCheckStatusCheck'));
    }
}
